  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = septentrio_gnss_driver::msg::GALAuthStatus;
    is_plain =
      (
      offsetof(DataType, gps_authentic_mask) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _GALAuthStatus__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_GALAuthStatus(full_bounded, is_plain, 0);
