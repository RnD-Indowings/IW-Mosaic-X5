  ...
  void * untyped_ros_message)
{
  ...
  // Field name: vector_info_geod
  {
    ...
    if (!septentrio_gnss_driver__msg__VectorInfoGeod__Sequence__init(&ros_message->vector_info_geod, size)) {
      fprintf(stderr, "failed to create array for field 'vector_info_geod'");
      return false;
    }
    auto array_ptr = ros_message->vector_info_geod.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = septentrio_gnss_driver__msg__BaseVectorGeod;
    is_plain =
      (
      offsetof(DataType, vector_info_geod) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _BaseVectorGeod__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_septentrio_gnss_driver__msg__BaseVectorGeod(
